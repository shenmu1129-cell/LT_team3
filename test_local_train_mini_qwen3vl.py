...
class QwenVLTrainer:
    ...
    def train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            # 前向传播
            results = self.model(images, pointclouds, mode='detect')
            
            # 计算损失
            predictions = torch.tensor([
                1.0 if r['is_attack'] else 0.0 for r in results
            ], device=self.device)
            ...
            # 二分类损失
            loss = self.criterion(
                confidences if predictions.sum() > 0 else torch.zeros_like(confidences),
                labels.float()
            )
    ...
    def validate(self):
        ...
        for batch in self.val_loader:
            ...
            predictions = torch.tensor([
                1 if r['is_attack'] else 0 for r in results
            ])
